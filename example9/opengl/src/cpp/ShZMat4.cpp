...
// ShZMat4
ShZMat4::ShZMat4(float e0, float e1, float e2, float e3,
...
ShZMat4::ShZMat4(const ShZMat4 & rhs)
{
	memcpy(entries, rhs.entries, 16*sizeof(float));
}

ShZMat4::ShZMat4(const float * rhs)
{
	memcpy(entries, rhs, 16*sizeof(float));
}

void ShZMat4::SetEntry(int position, float value)
{
	if(position>=0 && position<=15)
		entries[position]=value;
}
	
float ShZMat4::GetEntry(int position) const
{
	if(position>=0 && position<=15)
...
VECTOR4D ShZMat4::GetRow(int position) const
{
	if(position==0)
...
VECTOR4D ShZMat4::GetColumn(int position) const
{
	if(position==0)
...
void ShZMat4::LoadIdentity(void)
{
...
void ShZMat4::LoadZero(void)
{
	memset(entries, 0, 16*sizeof(float));
}

ShZMat4 ShZMat4::operator+(const ShZMat4 & rhs) const		//overloaded operators
{
	return ShZMat4(	entries[0]+rhs.entries[0],
...
ShZMat4 ShZMat4::operator*(const ShZMat4 & rhs) const
{
	...
	{
		return ShZMat4(	entries[0]*rhs.entries[0]+entries[4]*rhs.entries[1]+entries[8]*rhs.entries[2],
	...
	if(	entries[3]==0.0f && entries[7]==0.0f && entries[11]==0.0f && entries[15]==1.0f)
	{
		return ShZMat4(	entries[0]*rhs.entries[0]+entries[4]*rhs.entries[1]+entries[8]*rhs.entries[2]+entries[12]*rhs.entries[3],
...
ShZMat4 ShZMat4::operator*(const float rhs) const
{
	return ShZMat4(	entries[0]*rhs,
						entries[1]*rhs,
						entries[2]*rhs,
						entries[3]*rhs,
						entries[4]*rhs,
...
ShZMat4 operator*(float scaleFactor, const ShZMat4 & rhs)
{
	return rhs*scaleFactor;
}

bool ShZMat4::operator==(const ShZMat4 & rhs) const
{
	for(int i=0; i<16; i++)
	{
...
void ShZMat4::operator+=(const ShZMat4 & rhs)
{
	(*this)=(*this)+rhs;
}
...
void ShZMat4::operator*=(const float rhs)
{
	(*this)=(*this)*rhs;
}
...
ShZMat4 ShZMat4::operator-(void) const
{
	ShZMat4 result(*this);
...
VECTOR4D ShZMat4::operator*(const VECTOR4D rhs) const
{
...
VECTOR3D ShZMat4::operator*(const VECTOR3D rhs_) const
{
...
VECTOR3D ShZMat4::GetRotatedVector3D(const VECTOR3D & rhs) const
{
...
VECTOR3D ShZMat4::GetInverseRotatedVector3D(const VECTOR3D & rhs) const
{
	//rotate by transpose:
...
VECTOR3D ShZMat4::GetTranslatedVector3D(const VECTOR3D & rhs) const
{
	return VECTOR3D(rhs.x+entries[12], rhs.y+entries[13], rhs.z+entries[14]);
}

VECTOR3D ShZMat4::GetInverseTranslatedVector3D(const VECTOR3D & rhs) const
{
	return VECTOR3D(rhs.x-entries[12], rhs.y-entries[13], rhs.z-entries[14]);
}

void ShZMat4::Invert(void)
{
	*this=GetInverse();
}

ShZMat4 ShZMat4::GetInverse(void) const
{
	ShZMat4 result=GetInverseTranspose();
...
void ShZMat4::Transpose(void)
{
	*this=GetTranspose();
}

ShZMat4 ShZMat4::GetTranspose(void) const
{
	return ShZMat4(	entries[ 0], entries[ 4], entries[ 8], entries[12],
...
void ShZMat4::InvertTranspose(void)
{
	*this=GetInverseTranspose();
}

ShZMat4 ShZMat4::GetInverseTranspose(void) const
{
	ShZMat4 result;
...
void ShZMat4::AffineInvertTranspose(void)
{
	(*this)=GetAffineInverseTranspose();
}

ShZMat4 ShZMat4::GetAffineInverseTranspose(void) const
{
	//return the transpose of the rotation part
	//and the negative of the inverse rotated translation part
	//transposed
...
void ShZMat4::SetTranslation(const VECTOR3D & translation)
{
	LoadIdentity();
...
void ShZMat4::SetScale(const VECTOR3D & scaleFactor)
{
	LoadIdentity();
...
void ShZMat4::SetUniformScale(const float scaleFactor)
{
	LoadIdentity();
...
void ShZMat4::SetRotationAxis(const double angle, const VECTOR3D & axis)
{
	VECTOR3D u=axis.GetNormalized();
...
void ShZMat4::SetRotationX(const double angle)
{
	LoadIdentity();
...
void ShZMat4::SetRotationY(const double angle)
{
	LoadIdentity();
...
void ShZMat4::SetRotationZ(const double angle)
{
	LoadIdentity();
...
void ShZMat4::SetRotationEuler(const double angleX, const double angleY, const double angleZ)
{
	LoadIdentity();
...
void ShZMat4::SetPerspective(	float left, float right, float bottom,
...
void ShZMat4::SetPerspective(float fovy, float aspect, float n, float f)
{
...
void ShZMat4::SetOrtho(	float left, float right, float bottom,
							float top, float n, float f)
{
	LoadIdentity();
...
void ShZMat4::SetLookAt(float eyeX, float eyeY, float eyeZ, float centerX, float centerY, float centerZ, float upX, float upY, float upZ)
{
	...
	ShZMat4 transformMat;
	transformMat.SetTranslation(-eyeX,-eyeY,-eyeZ);

	*this = *this * transformMat;
}

void ShZMat4::SetTranslationPart(const VECTOR3D & translation)
{
...
void ShZMat4::SetRotationPartEuler(const double angleX, const double angleY, const double angleZ)
{
