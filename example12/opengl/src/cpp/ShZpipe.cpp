...
pipe::pipe()
{
	...
	m_positionY = 0;
	m_positionZ = 0;
	m_viewPosition = VECTOR3D(m_positionX - m_R_inner, m_positionY, m_positionZ);
	m_lightPosition = VECTOR3D(m_positionX - m_R_out * 2, m_positionY , m_positionZ + m_R_out * 2);
...
void pipe::getPosition(GLdouble &positionX, GLdouble &positionY, GLdouble &positionZ)
{
...
void pipe::setPosition(GLdouble positionX, GLdouble positionY, GLdouble positionZ)
{
...
void pipe::clear()
{
	...
	m_vertices_tempInner.clear();
	m_vertices_tempOut.clear();
	m_index_innerL.clear();
	m_index_innerR.clear();
	m_index_outL.clear();
	m_index_outR.clear();
	m_index_innerCompensation.clear();
	m_index_outCompensation.clear();
	glBindVertexArray(0);
...
void pipe::init()
{
	...
	{
		m_vertices_inner.push_back(VECTOR3D(m_positionX + m_R_inner * cos(i*offsetAngle), m_positionY + m_H/2, m_positionZ + m_R_inner * sin(i * offsetAngle)));
		m_normals_inner.push_back(VECTOR3D(cos(i*offsetAngle),0,sin(i * offsetAngle)));
		m_vertices_inner.push_back(VECTOR3D(m_positionX + m_R_inner * cos(i*offsetAngle), -m_H/2 + m_positionY, m_positionZ + m_R_inner * sin(i * offsetAngle)));  
		m_normals_inner.push_back(VECTOR3D(cos(i*offsetAngle),0,sin(i * offsetAngle)));
	}
	...
	for (int i=0; i<m_numberPoint; i++)
	{
		m_vertices_out.push_back(VECTOR3D(m_positionX + m_R_out * cos(i*offsetAngle), m_positionY + m_H/2, m_positionZ + m_R_out * sin(i * offsetAngle)));
		m_normals_out.push_back(VECTOR3D(cos(i*offsetAngle),0,sin(i * offsetAngle)));
		m_vertices_out.push_back(VECTOR3D(m_positionX + m_R_out * cos(i*offsetAngle), -m_H/2 + m_positionY, m_positionZ + m_R_out * sin(i * offsetAngle)));  
		m_normals_out.push_back(VECTOR3D(cos(i*offsetAngle),0,sin(i * offsetAngle)));
	}
	//m_vertices_temp
	...
	m_index_innerCompensation.push_back(m_numberPoint-2);
	m_index_innerCompensation.push_back(m_numberPoint-1);
	m_index_innerCompensation.push_back(m_numberPoint);
	m_index_innerCompensation.push_back(m_numberPoint+1);
	...
	m_index_outCompensation.push_back(m_numberPoint-2);
	m_index_outCompensation.push_back(m_numberPoint-1);
	m_index_outCompensation.push_back(m_numberPoint);
	m_index_outCompensation.push_back(m_numberPoint+1);
...
void pipe::draw_shape(ShZshader *pShader)
{
	...
	drawfaceInnerCompensation(pShader);
	drawfaceOutCompensation(pShader);
	pShader->end();
}
...
void pipe::unfoldInnerL()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		temp3.SetTranslation(VECTOR3D(0,0,m_R_inner));
		temp6.SetTranslation(VECTOR3D(0,0,-m_R_inner));
		...
		for (unsigned int i=0; i< m_index_innerL.size(); i+=2)
		{
			...
			temp4.SetRotationAxis(-m_angle_innerL[i>>1],VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(-m_R_inner * 2 * m_angle_innerL[i>>1] * PI / 180 + m_R_inner * 2 * sin(m_angle_innerL[i>>1] * PI / 180), 0, 0));
		...
		glBufferData(GL_ARRAY_BUFFER, m_normals_inner.size() * 3 * sizeof(GL_FLOAT),m_normals_inner.data(), GL_STATIC_DRAW);
...
void pipe::unfoldInnerR()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_innerR.size(); i+=2)
		{
			...
			temp4.SetRotationAxis(m_angle_innerR[i>>1],VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(m_R_inner * 2 * m_angle_innerR[i>>1] * PI / 180 - m_R_inner * 2 * sin(m_angle_innerR[i>>1] * PI / 180),0,0));
...
void pipe::unfoldOutL()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		temp2.SetRotationAxis(90,VECTOR3D(0,1,0));
		temp3.SetTranslation(VECTOR3D(0,0,m_R_out));
		temp6.SetTranslation(VECTOR3D(0,0,-m_R_out));
		...
		for (unsigned int i=0; i< m_index_outL.size(); i+=2)
		{
			...
			temp4.SetRotationAxis(-m_angle_outL[i>>1],VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(-m_R_out * 2 * m_angle_outL[i>>1] * PI / 180 + m_R_out * 2 * sin(m_angle_outL[i>>1] * PI / 180), 0, 0));
		...
		glBindBuffer(GL_ARRAY_BUFFER, m_nbo);
		glBufferData(GL_ARRAY_BUFFER, m_normals_out.size() * 3 * sizeof(GL_FLOAT),m_normals_out.data(), GL_STATIC_DRAW);
		glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0);
		glEnableVertexAttribArray(1);
...
void pipe::unfoldOutR()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_innerR.size(); i+=2)
		{
			...
			temp4.SetRotationAxis(m_angle_outR[i>>1],VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(m_R_out * 2 * m_angle_outR[i>>1] * PI / 180 - m_R_out * 2 * sin(m_angle_outR[i>>1] * PI / 180),0,0));
...
void pipe::foldInnerL()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		temp2.SetRotationAxis(90,VECTOR3D(0,1,0));
		temp3.SetTranslation(VECTOR3D(0,0,m_R_inner));
		temp8.SetTranslation(VECTOR3D(0,0,-m_R_inner));

		temp9.SetTranslation(VECTOR3D(m_positionX, m_positionY, m_positionZ));

		for (int i=0; i< (int)m_index_innerL.size(); i+=2)
		{
			temp4.SetRotationAxis(-(sizeFaceInnerL - 1 - i/2) * offsetAngle_l_degree,VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(-m_R_inner * 2 * (sizeFaceInnerL - 1 - i/2) * offsetAngle_l_degree * PI / 180 + m_R_inner * 2 * sin((sizeFaceInnerL - 1 - i/2) * offsetAngle_l_degree * PI / 180), 0, 0));

			temp6.SetTranslation(VECTOR3D(m_R_inner * 2 * m_angle_innerL[i>>1] * PI / 180 - m_R_inner * 2 * sin(m_angle_innerL[i>>1] * PI / 180), 0, 0));
			temp7.SetRotationAxis(m_angle_innerL[i>>1],VECTOR3D(0,1,0));

			m_vertices_inner[m_index_innerL[i]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempInner[m_index_innerL[i]];
			m_vertices_inner[m_index_innerL[i+1]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempInner[m_index_innerL[i+1]];
		}
...
void pipe::foldInnerR()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		for (int i=0; i< (int)m_index_innerR.size(); i+=2)
		{
			...
			temp5.SetTranslation(VECTOR3D(m_R_inner * i * offsetAngle_r_degree * PI / 180 - m_R_inner * 2 * sin(i/2 * offsetAngle_r_degree * PI / 180), 0, 0));

			temp6.SetTranslation(VECTOR3D(-m_R_inner * 2 * m_angle_innerR[i>>1] * PI / 180 + m_R_inner * 2 * sin(m_angle_innerR[i>>1] * PI / 180), 0, 0));
			temp7.SetRotationAxis(-m_angle_innerR[i>>1],VECTOR3D(0,1,0));

			m_vertices_inner[m_index_innerR[i]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempInner[m_index_innerR[i]];
			m_vertices_inner[m_index_innerR[i+1]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempInner[m_index_innerR[i+1]];
		}
...
void pipe::foldOutL()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		temp2.SetRotationAxis(90,VECTOR3D(0,1,0));
		temp3.SetTranslation(VECTOR3D(0,0,m_R_out));
		temp8.SetTranslation(VECTOR3D(0,0,-m_R_out));

		temp9.SetTranslation(VECTOR3D(m_positionX, m_positionY, m_positionZ));

		for (int i=0; i< (int)m_index_outL.size(); i+=2)
		{
			temp4.SetRotationAxis(-(sizeFaceOutL - 1 - i/2) * offsetAngle_l_degree,VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(-m_R_out * 2 * (sizeFaceOutL - 1 - i/2) * offsetAngle_l_degree * PI / 180 + m_R_out * 2 * sin((sizeFaceOutL - 1 - i/2) * offsetAngle_l_degree * PI / 180), 0, 0));

			temp6.SetTranslation(VECTOR3D(m_R_out * 2 * m_angle_outL[i>>1] * PI / 180 - m_R_out * 2 * sin(m_angle_outL[i>>1] * PI / 180), 0, 0));
			temp7.SetRotationAxis(m_angle_outL[i>>1],VECTOR3D(0,1,0));

			m_vertices_out[m_index_outL[i]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempOut[m_index_outL[i]];
			m_vertices_out[m_index_outL[i+1]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempOut[m_index_outL[i+1]];
		}
...
void pipe::foldOutR()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		{
			temp4.SetRotationAxis(i/2 * offsetAngle_r_degree,VECTOR3D(0,1,0));
			temp5.SetTranslation(VECTOR3D(m_R_out * i * offsetAngle_r_degree * PI / 180 - m_R_out * 2 * sin(i/2 * offsetAngle_r_degree * PI / 180), 0, 0));

			temp6.SetTranslation(VECTOR3D(-m_R_out * 2 * m_angle_outR[i>>1] * PI / 180 + m_R_out * 2 * sin(m_angle_outR[i>>1] * PI / 180), 0, 0));
			temp7.SetRotationAxis(-m_angle_outR[i>>1],VECTOR3D(0,1,0));

			m_vertices_out[m_index_outR[i]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempOut[m_index_outR[i]];
			m_vertices_out[m_index_outR[i+1]] = temp9 * temp8 * temp7 * temp6 * temp5 * temp4 * temp3 * temp2 * temp1 * m_vertices_tempOut[m_index_outR[i+1]];
		}
...
void pipe::drawfaceOutL(ShZshader *pShader)
{
	...
	glDrawElements(GL_TRIANGLE_STRIP,m_index_outL.size(),GL_UNSIGNED_INT,0);
...
void pipe::drawfaceOutR(ShZshader *pShader)
{
	...
	glDrawElements(GL_TRIANGLE_STRIP,m_index_outR.size(),GL_UNSIGNED_INT,0);
...
void pipe::drawfaceInnerL(ShZshader *pShader)
{
	...
	glDrawElements(GL_TRIANGLE_STRIP,m_index_innerL.size(),GL_UNSIGNED_INT,0);
...
void pipe::drawfaceInnerR(ShZshader *pShader)
{
	...
	glDrawElements(GL_TRIANGLE_STRIP,m_index_innerR.size(),GL_UNSIGNED_INT,0);
...
void pipe::drawfaceInnerCompensation(ShZshader *pShader)
{
	if (state == CLOSED)
	{
		glGenVertexArrays(1,&m_vao);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER,m_index_innerCompensation.size() * sizeof(GL_UNSIGNED_INT), m_index_innerCompensation.data(), GL_STATIC_DRAW);

		modelLocaiton = glGetUniformLocation(pShader->programShader,"model");
		glUniformMatrix4fv(modelLocaiton, 1, false, m_model_innerCompensation);
		...
		glDrawElements(GL_TRIANGLE_STRIP,m_index_innerCompensation.size(),GL_UNSIGNED_INT,0);
...
void pipe::drawfaceOutCompensation(ShZshader *pShader)
{
	if (state == CLOSED)
	{
		glGenVertexArrays(1,&m_vao);
		...
		glBufferData(GL_ELEMENT_ARRAY_BUFFER,m_index_outCompensation.size() * sizeof(GL_UNSIGNED_INT), m_index_outCompensation.data(), GL_STATIC_DRAW);

		modelLocaiton = glGetUniformLocation(pShader->programShader,"model");
		glUniformMatrix4fv(modelLocaiton, 1, false, m_model_outCompensation);
		...
		glDrawElements(GL_TRIANGLE_STRIP,m_index_outCompensation.size(),GL_UNSIGNED_INT,0);
