...
void cylinder::unfoldsl()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_sidel.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(-m_R * m_angle_sl[i>>1] * PI / 180 + 2 * m_R * sin(m_angle_sl[i>>1] * PI / 180), 0, 0));
...
void cylinder::unfoldsr()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_sider.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(m_R *  m_angle_sr[i>>1] * PI / 180 - 2 * m_R * sin(m_angle_sr[i>>1] * PI / 180),0,0));
...
void cylinder::foldsl()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		for (int i=0; i< (int)m_index_sidel.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(-m_R * (sizeFacel - 1 - i/2) * offsetAngle_l_degree * PI / 180 + 2 * m_R * sin ((sizeFacel - 1 - i/2) * offsetAngle_l_degree * PI / 180), 0, 0));

			temp5.SetTranslation(VECTOR3D(m_R * m_angle_sl[i>>1] * PI / 180 - 2 * m_R * sin(m_angle_sl[i>>1] * PI / 180), 0, 0));
...
void cylinder::foldsr()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		for (int i=0; i< (int)m_index_sider.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(m_R * i/2 * offsetAngle_r_degree * PI / 180 - 2 * m_R * sin(i/2 * offsetAngle_r_degree * PI / 180), 0, 0));

			temp5.SetTranslation(VECTOR3D(-m_R * m_angle_sr[i>>1] * PI / 180 + 2 * m_R * sin(m_angle_sr[i>>1] * PI / 180), 0, 0));
