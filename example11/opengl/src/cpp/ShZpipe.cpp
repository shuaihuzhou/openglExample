...
void pipe::unfoldInnerL()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_innerL.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(-m_R_inner * m_angle_innerL[i>>1] * PI / 180 + 2 * m_R_inner * sin(m_angle_innerL[i>>1] * PI / 180), 0, 0));
...
void pipe::unfoldInnerR()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_innerR.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(m_R_inner *  m_angle_innerR[i>>1] * PI / 180 - 2 * m_R_inner * sin(m_angle_innerR[i>>1] * PI / 180),0,0));
...
void pipe::unfoldOutL()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_outL.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(-m_R_out * m_angle_outL[i>>1] * PI / 180 + 2 * m_R_out * sin(m_angle_outL[i>>1] * PI / 180), 0, 0));
...
void pipe::unfoldOutR()
{
	...
	if ((m_current - m_start) < CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) >= 0)
	{
		...
		for (unsigned int i=0; i< m_index_innerR.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(m_R_out *  m_angle_outR[i>>1] * PI / 180 - 2 * m_R_out * sin(m_angle_outR[i>>1] * PI / 180),0,0));
...
void pipe::foldInnerL()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		for (int i=0; i< (int)m_index_innerL.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(-m_R_inner * (sizeFaceInnerL - 1 - i/2) * offsetAngle_l_degree * PI / 180 + 2 * m_R_inner * sin((sizeFaceInnerL - 1 - i/2) * offsetAngle_l_degree * PI / 180), 0, 0));

			temp5.SetTranslation(VECTOR3D(m_R_inner * m_angle_innerL[i>>1] * PI / 180 - 2 * m_R_inner * sin(m_angle_innerL[i>>1] * PI / 180), 0, 0));
...
void pipe::foldInnerR()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		for (int i=0; i< (int)m_index_innerR.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(m_R_inner * i/2 * offsetAngle_r_degree * PI / 180 - 2 * m_R_inner * sin(i/2 * offsetAngle_r_degree * PI / 180), 0, 0));

			temp5.SetTranslation(VECTOR3D(-m_R_inner * m_angle_innerR[i>>1] * PI / 180 + 2 * m_R_inner * sin(m_angle_innerR[i>>1] * PI / 180), 0, 0));
...
void pipe::foldOutL()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		for (int i=0; i< (int)m_index_outL.size(); i+=2)
		{
			...
			temp4.SetTranslation(VECTOR3D(-m_R_out * (sizeFaceOutL - 1 - i/2) * offsetAngle_l_degree * PI / 180 + 2 * m_R_out * sin((sizeFaceOutL - 1 - i/2) * offsetAngle_l_degree * PI / 180), 0, 0));

			temp5.SetTranslation(VECTOR3D(m_R_out * m_angle_outL[i>>1] * PI / 180 - 2 * m_R_out * sin(m_angle_outL[i>>1] * PI / 180), 0, 0));
...
void pipe::foldOutR()
{
	...
	if ((m_current - m_start) < 2 * CLOCKS_PER_SEC * m_totalTime && (m_current - m_start) > CLOCKS_PER_SEC * m_totalTime)
	{
		...
		{
			temp3.SetRotationAxis(i/2 * offsetAngle_r_degree,VECTOR3D(0,1,0));
			temp4.SetTranslation(VECTOR3D(m_R_out * i/2 * offsetAngle_r_degree * PI / 180 - 2 * m_R_out * sin(i/2 * offsetAngle_r_degree * PI / 180), 0, 0));

			temp5.SetTranslation(VECTOR3D(-m_R_out * m_angle_outR[i>>1] * PI / 180 + 2 * m_R_out * sin(m_angle_outR[i>>1] * PI / 180), 0, 0));
